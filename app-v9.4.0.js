...
const activities = [
  ...
  { id: "cleanup", name: "おかたづけ", image: "cleanup.png" }
];
...
const activitySound = new Audio("activity-change.mp3?v=9.4.0");
const warningSound = new Audio("mei-kara-mei-switch2.mp3?v=9.4.0");
const personalSound = new Audio("personal-alarm.mp3?v=9.4.0");
...
function updateMain() {
  ...
  if (diff > 0) {
    // 設定した「○分前」に入り、ビジュアルタイマーのカウントが
    // 始まった瞬間に、新しい予告音を1回だけ再生する。
    if (warningKey !== k && warningAttemptKey !== k) {
      warningAttemptKey = k;
      soundWarning().then(played => {
        warningAttemptKey = "";
        if (played) warningKey = k;
      });
    }
...
async function playFileSound(player, fallback) {
  try {
    player.pause();
...
async function unlockMediaAudio() {
  ...
  const players = [activitySound, warningSound, personalSound];
  try {
...
function soundWarning() {
  return playFileSound(warningSound, fallbackWarning);
}
...
function soundActivity() {
  return playFileSound(activitySound, fallbackActivity);
}

function soundPersonal() {
  return playFileSound(personalSound, fallbackPersonal);
}
...
function saveNext(e) {
  ...
  if (!state.nextId) state.nextStart = "";
  transitionKey = "";
  warningKey = "";
  warningAttemptKey = "";
  save();
...
function init() {
  // 先生が最初に画面へ触れた時点で音源を解放し、
  // 時刻到達による自動再生がブラウザに止められないようにする。
  const unlock = () => unlockMediaAudio();
  ...
  $("mainTimerButton").onclick = openTimerDialog;
  $("mainTimerButton").onkeydown = event => {
